...
function mergeTokenClaimsIntoProfile (profile: UserProfile, idToken: string): UserProfile {
  const claims = decodeJwtPayload(idToken)
  return {
    ...claims,
    ...profile,
    app_metadata: profile.app_metadata || claims.app_metadata || claims['http://datatools/app_metadata'],
    user_metadata: profile.user_metadata || claims.user_metadata || claims['http://datatools/user_metadata']
  }
}
...
const ActiveUserRetriever = ({ accountTypes, logout, receiveTokenAndProfile }: Props) => {
  ...
  useEffect(async () => {
    if (isAuthenticated) {
      ...
      const profileWithClaims = mergeTokenClaimsIntoProfile(profile, tokenResponse.id_token)
      receiveTokenAndProfile({ profile: profileWithClaims, token: tokenResponse.id_token })

      // After receiving profile, also detect whether to show terms and conditions for the user account.
      showLicenseIfNeeded(accountTypes, profileWithClaims)
    } else if (!isLoading) {
      if (error) {
