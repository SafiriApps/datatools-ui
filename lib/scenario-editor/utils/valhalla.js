...
function latLngToCoordinate (point: LatLng) {
  return [point.lng, point.lat]
}

function straightSegmentFromLatLngs (points: Array<LatLng>): Coordinates {
  return points.map(latLngToCoordinate)
}

async function routeIndividualLegs (
  points: Array<LatLng>,
  avoidMotorways: boolean = false,
  followRail: boolean = false
): Promise<Array<Coordinates>> {
  const segments = []
  for (let i = 0; i < points.length - 1; i++) {
    const legPoints = points.slice(i, i + 2)
    try {
      const json = await routeWithGraphHopper(legPoints, avoidMotorways, followRail)
      const path = json && json.paths && json.paths[0]
      if (path) {
        const segment = handleGraphHopperRouting(path, false)
      ...
      console.warn(`Error routing individual leg ${i}`, e)
    }
    segments.push(straightSegmentFromLatLngs(legPoints))
  }
  return segments
}
...
export async function polyline (
  ...
  try {
    ...
    // Default to chunks of 5 points if the point limit is less than 2. GraphHopper's
    // hosted API allows at most 5 waypoints per request. (There must be at least
    // two points passed in to routing method in order to successfully route.)
    const chunk = pointLimit > 2 ? pointLimit : 5
    const chunkStep = chunk - 1
    for (let beginIndex = 0; beginIndex < points.length - 1; beginIndex += chunkStep) {
      // Overlap chunks by one point so each inter-stop leg is preserved.
      const endIndex = Math.min(beginIndex + chunk, points.length)
      ...
      try {
        ...
        console.warn(`Error routing from point ${beginIndex} to ${endIndex}`, e, chunkedPoints)
        if (individualLegs) {
          const legSegments = await routeIndividualLegs(chunkedPoints, avoidMotorways, followRail)
          geometry.push(...legSegments)
          continue
        }
        return null
      }
      ...
      if (path) {
        ...
        if (individualLegs && result.length !== chunkedPoints.length - 1) {
          console.warn(`Expected ${chunkedPoints.length - 1} routed legs but got ${result.length}. Routing each leg separately.`)
          const legSegments = await routeIndividualLegs(chunkedPoints, avoidMotorways, followRail)
          geometry.push(...legSegments)
          continue
        }
        ...
        if (individualLegs) {
          ...
          // If routing an unsplit line fails, let callers decide whether to
